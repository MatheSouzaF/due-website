...
// MENUS
function dz_deregister_menu()
{
  //Remover suporte a menu
}
add_action('init', 'dz_deregister_menu');
...
function custom_setup()
{
  ...
  // Languages
  load_theme_textdomain('dz_2024', get_template_directory() . '/languages');
  ...
  // Versionamento de assets atualizado no build do tema.
  // Deve ser utilizado no enqueue style/script buildado
  if (file_exists(__DIR__ . '/../assets_version.php')) {
    include(__DIR__ . '/../assets_version.php');
  } else {
...
// Filtro para mostrar o ACF "titulo_da_pagina" como título público, para que o
// post_title seja utilizado internamente com código interno para melhor organização
add_filter('the_title', function ($post_title, $post_id) {
  if (!is_admin() && isset($post_id) && get_field('titulo_da_pagina', $post_id)) {
    $post_title = get_field('titulo_da_pagina', $post_id);
  }
  return $post_title;
}, 10, 2);

// Regra Temporária: para exportação de dados ACF
// BEGIN
add_action('init', 'add_query_var_extract_acf');
function add_query_var_extract_acf()
{
  global $wp;
